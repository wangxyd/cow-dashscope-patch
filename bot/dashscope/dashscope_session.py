    ...
    def discard_exceeding(self, max_tokens, cur_tokens=None):
        ...
        while cur_tokens > max_tokens:
            if len(self.messages) > 2:
                discard_element = self.messages.pop(1)
                # START: 避免以system、assistant开头的问题
                logger.debug(f"The second element has been discard because of exceeding max_tokens, discard_element: {discard_element}")
                if self.messages[1]["role"] == "assistant":
                    discard_element = self.messages.pop(1)
                    logger.debug(f"The third element has also been discard because of assistant role, discard_element: {discard_element}")
                # END
