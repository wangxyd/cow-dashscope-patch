        ...
        if content.startswith("#"):
            ...
            if any(cmd in info["alias"] for info in COMMANDS.values()):
                ...
                if cmd == "auth":
                    ...
                    logger.debug(f"[Godcmd] cmd: {repr(cmd)}, args: {repr(args)}, len(args): {len(args)}")
                    if len(args) == 0:
                    ...
                    if bottype in [const.OPEN_AI, const.CHATGPT, const.CHATGPTONAZURE, const.LINKAI, const.BAIDU, const.XUNFEI, const.QWEN, const.QWEN_DASHSCOPE, const.GEMINI, const.ZHIPU_AI]:
                ...
                logger.debug(f"[Godcmd] self.admin_users: {self.admin_users}, user: {user}, isadmin: {isadmin}")
